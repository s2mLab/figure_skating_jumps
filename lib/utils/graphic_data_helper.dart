...
class GraphicDataHelper {
  static List<ValueDatePair> getJumpScorePerTypeGraphData(
      Map<String, List<Capture>> captures, JumpType type) {
  ...
  static List<ValueDatePair> getPercentageSucceededGraphData(
  ...
  static List<ValueDatePair> getAverageFlyTimeGraphData(
      Map<String, List<Capture>> captures) {
    List<ValueDatePair> graphData = [];
    List<String> dates = _getAllDates(captures);
    for (String day in dates) {
  ...
  static double? _getAverageJumpScorePerTypeOnDate(
      Map<String, List<Capture>> captures, String day, JumpType type) {
    ...
    List<Jump> jumpsOfType = _getRequiredJumpsFromCaptures(
  ...
  static double? _getPercentageOfSuccessfulJumpsOnDate(
      Map<String, List<Capture>> captures, String day) {
    if (captures[day] == null || captures[day]!.isEmpty) {
      throw ArgumentError('captures at date was null or empty');
    }
    List<Capture> capturesOnDate = captures[day]!;
  ...
  static double? _getAverageJumpDurationOnDate(
      Map<String, List<Capture>> captures, String day) {
    if (captures[day] == null || captures[day]!.isEmpty) {
      throw ArgumentError('captures at date was null or empty');
    }
  ...
  static List<Jump> _getRequiredJumpsFromCaptures(List<Capture> captures,
      [bool Function(Jump)? test]) {
    List<Jump> jumps = [];
